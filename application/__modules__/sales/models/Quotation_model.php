...
{
	/* Module */
 	protected $folder_name			= "sales/quotation";
 	protected $table_name 			= _PREFIX_TABLE."data_quotation";
 	protected $table_customer 		= _PREFIX_TABLE."data_customer";
 	protected $table_rab 			= _PREFIX_TABLE."data_rab";
 	...
 	protected $table_valid 			= _PREFIX_TABLE."option_term_of_validity";
	...
	public function get_list_data()
	{
		...
		$sTable = ' '.$this->table_name.' a
					...
					LEFT JOIN '.$this->table_rab.' c ON c.id=a.id_rab
					LEFT JOIN '.$this->table_project.' d ON d.id=c.id_project
					LEFT JOIN '.$this->table_karyawan.' e ON e.id=a.id_ttd
					';
	...
	public function add_data($post) {
		...
		$lettercode = $this->lettercode('8'); // quotation code
		...
		$id_ttd = (isset($post['id_ttd']) && !empty($post['id_ttd']))? trim($post['id_ttd']):NULL;
		$data = [
			...
			'id_valid' 			=> $id_valid,
			'to_contact' 		=> trim($post['to_contact']),
			'id_ttd' 			=> $id_ttd,
	...
	public function getRowData($id) { 
		...
		if(!empty($rs->id_rab)){
			...
			$rs = (object) array_merge((array) $rs, ['id_project'=>'','total_budget'=>'0','project'=>'','project_title'=>'']);
		}
		...
		if(!empty($rs->id_valid)){
			...
			$rs = (object) array_merge((array) $rs, ['valid'=>'']);
		}
		if(!empty($rs->id_ttd)){
			$rd = $this->db->select('a.name as ttd, b.description as jabatan')->join($this->table_jabatan.' b', 'b.id = a.id_jabatan', 'left')->where(['a.'.$this->primary_key => $rs->id_ttd])->get($this->table_karyawan.' a')->row();
			$rs = (object) array_merge((array) $rs, (array) $rd);
		} else {
			$rs = (object) array_merge((array) $rs, ['ttd'=>'','jabatan'=>'']);
		}
	...
	public function import_data($list_data)
	{
		...
		foreach ($list_data as $k => $v) {
			...
			$id_ttd = !empty($v["K"])? trim($v["K"]):NULL;
			$data = [
				'quotation' 		=> trim($v["B"]),
				'id_customer' 		=> $id_customer,
