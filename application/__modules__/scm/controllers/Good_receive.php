...
{
	/* Costs */
 	const  LABELMODULE				= "good_receive"; // identify menu
 	const  LABELMASTER				= "Good Receive";
 	const  LABELFOLDER				= "scm"; // module folder
 	const  LABELPATH				= "good_receive"; // controller file (lowercase)
	...
	public $tabel_header 			= ["ID","No DO","Date Receive","PO","Note"];
	
	/* Export */
	public $colnames 				= ["ID","DO","Date Receive","PO","Note"];
	...
	public function form_field_asset()
	{
        ...
        $field['txtdate'] 			= $this->self_model->return_build_txtdate(date('d-m-Y'),'date_receive','date_receive');
		...
		$field['seloReceived'] 		= $this->self_model->return_build_select2me($oKaryawan,'','','','received','received','','','id','karyawan','-','name');
		$field['seloShipped'] 		= $this->self_model->return_build_select2me($oKaryawan,'','','','shipped','shipped','','','id','karyawan','-','name');
		$field['seloPrepared'] 		= $this->self_model->return_build_select2me($oKaryawan,'','','','prepared','prepared','','','id','karyawan','-','name');
		$field['seloAuthorized'] 	= $this->self_model->return_build_select2me($oKaryawan,'','','','authorized','authorized','','','id','karyawan','-','name');
		$field['txtnote'] 			= $this->self_model->return_build_txtarea('','note_order','note_order');

		
        $field['txtshpname'] 		= $this->self_model->return_build_txt('','shp_name','shp_name');
        $field['txtshpaddress'] 	= $this->self_model->return_build_txtarea('','shp_address','shp_address');
        $field['txtshptelp'] 		= $this->self_model->return_build_txt('','shp_telp','shp_telp');
		return $field;
	}
