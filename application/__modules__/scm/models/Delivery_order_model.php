...
{
	/* Module */
 	protected $folder_name			= "scm/delivery_order";
 	protected $table_po     		= _PREFIX_TABLE."data_po";
 	protected $table_name 			= _PREFIX_TABLE."data_delivery_order";
	...
	public function get_list_data()
	{
		$aColumns = [
			...
			'a.id',
			'a.do',
			'DATE_FORMAT(FROM_UNIXTIME(a.date), "%d-%m-%Y") as dt',
			'b.po as po',
			'a.note'
		];
	...
	public function add_data($post)
	{
		...
		$received_by = (isset($post['received_by']) && !empty($post['received_by'])) ? trim($post['received_by']) : NULL;
		$prepared_by = (isset($post['prepared_by']) && !empty($post['prepared_by'])) ? trim($post['prepared_by']) : NULL;
		$shipped_by = (isset($post['shipped_by']) && !empty($post['shipped_by'])) ? trim($post['shipped_by']) : NULL;
		$authorized_by = (isset($post['authorized_by']) && !empty($post['authorized_by'])) ? trim($post['authorized_by']) : NULL;
		$data = [
			'do' 				=> trim($post['do']),
			'date' 				=> $this->date_to_int(trim($post['date'])),
			'id_po' 		    => $id_po,
			'id_project' 		=> $id_project,
			'recipient_name' 	=> trim($post['rec_name']),
			'recipient_address' => trim($post['rec_address']),
			'recipient_telp' 	=> trim($post['rec_telp']),
	...
	public function getRowData($id) { 
		$rs = $this->db->select('*,DATE_FORMAT(FROM_UNIXTIME(date), "%d-%m-%Y") as dt')->where([$this->primary_key => $id])->get($this->table_name)->row();
		if(!empty($rs->id_po)){
		...
		if(!empty($rs->received_by)){
			$rd = $this->db->select('name as received')->where([$this->primary_key => $rs->received_by])->get($this->table_karyawan)->row();
			$rs = (object) array_merge((array) $rs, (array) $rd);
		} else {
			$rs = (object) array_merge((array) $rs, ['received'=>'']);
		}
		if(!empty($rs->prepared_by)){
			$rd = $this->db->select('name as prepared')->where([$this->primary_key => $rs->prepared_by])->get($this->table_karyawan)->row();
			$rs = (object) array_merge((array) $rs, (array) $rd);
		} else {
			$rs = (object) array_merge((array) $rs, ['prepared'=>'']);
		}
		if(!empty($rs->shipped_by)){
			$rd = $this->db->select('name as shipped')->where([$this->primary_key => $rs->shipped_by])->get($this->table_karyawan)->row();
			$rs = (object) array_merge((array) $rs, (array) $rd);
		} else {
			$rs = (object) array_merge((array) $rs, ['shipped'=>'']);
		}
		if(!empty($rs->authorized_by)){
			$rd = $this->db->select('name as authorized')->where([$this->primary_key => $rs->authorized_by])->get($this->table_karyawan)->row();
			$rs = (object) array_merge((array) $rs, (array) $rd);
		} else {
			$rs = (object) array_merge((array) $rs, ['authorized'=>'']);
		}
