...
class Sadmin_tools_access_menu_role extends CI_Controller
{
	/* Module */
 	private $folder_name			= "general_system/sadmin_tools_access_menu_role";
 	private $module_name			= "general_system_sadmin_tools_access_menu_role";
 	private $table_name 			= _PREFIX_TABLE."user_group";
 	private $primary_key 			= "id";
 	private $model_name				= "sadmin_tools_access_menu_role_model";
	/* Navigation */
 	private $parent_menu			= "general_system";
 	private $subparent_menu			= "general_system_sadmin";
 	private $sub_menu 				= "general_system_sadmin_tools_access_menu_role";
	/* Label */
 	private $label_parent_modul		= "General System";
 	private $label_subparent_modul	= "Super Admin Tools";
 	private $label_modul			= "Role Access Menu";
 	private $label_list_data		= "Daftar Data Role Access Menu";
 	private $label_add_data			= "Tambah Data Role Access Menu";
 	private $label_update_data		= "Edit Data Role Access Menu";
 	private $label_sukses_disimpan 	= "Data berhasil disimpan";
 	private $label_gagal_disimpan 	= "Data gagal disimpan";
 	private $label_delete_data		= "Hapus Data Role Access Menu";
 	private $label_sukses_dihapus 	= "Data berhasil dihapus";
 	private $label_gagal_dihapus 	= "Data gagal dihapus";
 	private $label_detail_data		= "Detail Data Role Access Menu";
 	private $label_import_data		= "Import Data Role Access Menu";
   	...
   	public function __construct()
	{
		...
		# akses level
		$akses = $this->sadmin_tools_access_menu_role_model->user_akses($this->module_name);
	...
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1") {
			$this->sadmin_tools_access_menu_role_model->get_list_data();
		} else {
			$this->load->view('errors/html/error_hacks_401');
		}
	}
	...
	public function delete($id)
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1" && _USER_ACCESS_LEVEL_DELETE == "1") {
			$rs = $this->sadmin_tools_access_menu_role_model->delete($id);
	...
	public function add()
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1" && _USER_ACCESS_LEVEL_ADD == "1") {
			...
			{
				$rs = $this->sadmin_tools_access_menu_role_model->add_data($post);
				...
				$sQuery 					= "SELECT name, description, id FROM "._PREFIX_TABLE."user_group WHERE 1 = 1 AND id NOT IN ( SELECT * FROM ( SELECT role_id FROM "._PREFIX_TABLE."user_akses_role GROUP BY role_id HAVING COUNT(*) >= 1 ) AS subquery )";
				$oUserAccount 				= $this->db->query($sQuery)->result();
				$this->data['selrole'] 		= $this->sadmin_tools_access_menu_role_model->return_build_select2me($oUserAccount,'','','','user_id','user_id','','','id','name','-','description');
	...
	public function edit($id)
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1" && _USER_ACCESS_LEVEL_UPDATE == "1") {
			if ($post = $this->input->post())
			{
				$rs = $this->sadmin_tools_access_menu_role_model->edit_data($post,$id);
				...
				$rs 						= $this->sadmin_tools_access_menu_role_model->getRowData($id);
				$sQuery 					= "SELECT name, description, id FROM "._PREFIX_TABLE."user_group WHERE 1 = 1 AND id IN ( SELECT * FROM ( SELECT role_id FROM "._PREFIX_TABLE."user_akses_role GROUP BY role_id HAVING COUNT(*) >= 1 ) AS subquery )";
				$oUserAccount 				= $this->db->query($sQuery)->result();
				$this->data['selrole'] 		= $this->sadmin_tools_access_menu_role_model->return_build_select2me($oUserAccount,'',$rs->id,'','user_id','user_id','','','id','name','-','description','disabled');
				$rs 						= $this->sadmin_tools_access_menu_role_model->getRowDataAccess($id);
				if (count($rs) > 0) {
					FOREACH ($rs AS $r) {
	...
	public function import_action()
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1" && _USER_ACCESS_LEVEL_IMPORT == "1") {
			...
			if (! $this->upload->do_upload("userfile")) {
				...
				# upload to database
				$rs = $this->sadmin_tools_access_menu_role_model->import_data($tmp_data);
			}
	...
	public function eksport_action()
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1" && _USER_ACCESS_LEVEL_EKSPORT == "1") {
			...
			$data 	= $this->sadmin_tools_access_menu_role_model->eksport_data();
			...
			$this->sadmin_tools_access_menu_role_model->export_to_excel($colnames,$colfields, $data, $header ,"rpt_sadmin_tools_access_menu_role");
		} else {
			$this->load->view('errors/html/error_hacks_401');
		}
	}
