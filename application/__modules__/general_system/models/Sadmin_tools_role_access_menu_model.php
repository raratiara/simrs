...
class Sadmin_tools_user_access_menu_model extends MY_Model
{
	/* Module */
 	protected $folder_name			= "general_system/sadmin_tools_user_access_menu";
 	...
 	protected $table_name2 			= _PREFIX_TABLE."user_akses";
	...
	public function delete($id= "") {
		if (isset($id) && $id <> "") {
			$rs = $this->db->where([$this->primary_key => $id])->delete($this->table_name2);
			return $rs;
		} else return null;
	}  
	...
	public function edit_data($post,$id) { 
		...
		if (isset($id) && $id <> "") {			
			...
			if (count($rs1) > 0) {
				...
				//$this->db->trans_off(); // Disable transaction
				$this->db->trans_start(); // set "True" for query will be rolled back				
				$this->db->where([$this->primary_key => $id])->delete($this->table_name2);
				if(count($itemData)>0){
					$this->db->insert_batch($this->table_name2,$itemData);
				}
				$this->db->trans_complete();
	...
	public function getRowDataAccess($id) { 
		return $rs = $this->db->where([$this->primary_key => $id])->get($this->table_name2)->result_array();
	} 
