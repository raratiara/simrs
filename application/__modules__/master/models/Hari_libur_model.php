...
class Hari_libur_model extends MY_Model
{
	/* Module */
 	protected $folder_name			= "master/hari_libur";
 	protected $table_name 			= _PREFIX_TABLE."data_hari_libur";
	...
	public function get_list_data()
	{
		$aColumns = [
			...
			NULL,
			'id',
			'DATE_FORMAT(FROM_UNIXTIME(date_libur), "%Y-%m-%d") as dlibur',
			'description',
			'notes', 
			'DATE_FORMAT(FROM_UNIXTIME(date_libur), "%d-%m-%Y") as tgl_libur'
		];
	...
	public function add_data($post) {
		$data = [
			'date_libur' 		=> $this->date_to_int(trim($post['date_libur'])),
	...
	public function getRowData($id) { 
		$rs = $this->db->select('*,DATE_FORMAT(FROM_UNIXTIME(date_libur), "%d-%m-%Y") as tgl_libur')->where([$this->primary_key => $id])->get($this->table_name)->row();
	...
	public function import_data($list_data)
	{
		...
		foreach ($list_data as $k => $v) {
			...
			$data = [
				'date_libur' 		=> $this->date_to_int(trim($v["B"])),
				'description' 		=> trim($v["C"]),
				'notes' 			=> trim($v["D"]),
				'insert_by'			=> $_SESSION["username"]
			];
