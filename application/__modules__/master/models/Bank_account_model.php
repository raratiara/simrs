...
{
	/* Module */
 	protected $folder_name			= "master/bank_account";
 	protected $table_name 			= _PREFIX_TABLE."option_bank_account";
	...
	public function get_list_data()
	{
		...
		$sTable = ' '.$this->table_name.' a 
					LEFT JOIN '.$this->table_bank.' b ON a.id_bank = b.id
					';
	...
	public function add_data($post) {
		...
		$data = [
			'id_bank' 			=> trim($post['id_bank']),
			'cabang' 			=> trim($post['cabang']),
			'name' 				=> trim($post['name']),
			'rekening' 			=> trim($post['rekening']),
			'active' 			=> $active,
	...
	public function eksport_data()
	{
		$sql = "SELECT
					...
					IF(a.active='1','Yes','No') as is_active
			FROM
	   	 		".$this->table_name." a LEFT JOIN ".$this->table_bank." b ON a.id_bank = b.id 
	   		ORDER BY
