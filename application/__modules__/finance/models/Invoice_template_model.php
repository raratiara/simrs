...
class Invoice_template_model extends MY_Model
{
	/* Module */
 	protected $folder_name			= "finance/invoice_template";
 	protected $table_name 			= _PREFIX_TABLE."data_invoice_template";
	...
	// Generate new invoice template item row
	public function getNewTInvoiceItemRow($row,$id=0,$view=FALSE)
	{ 
		if($id > 0){
			$data = $this->getTInvoiceItemRows($id,$view);
		} else {
			$data = '';
			$no = $row+1;
	...
	// Generate invoice template item rows for edit & view
	public function getTInvoiceItemRows($id,$view,$print=FALSE){
		$dt = '';
