...
class Invoice_template extends MYP_Controller
{
	/* Costs */
 	const  LABELMODULE				= "finance_invoice_template"; // identify menu
 	const  LABELMASTER				= "Invoice Template";
 	const  LABELFOLDER				= "finance"; // module folder
 	const  LABELPATH				= "invoice_template"; // controller file (lowercase)
	...
	public $tabel_header 			= ["ID","Template Name","Term","Bill To","Grand Total"];
	
	/* Export */
	public $colnames 				= ["ID","Template Name","PO","Term","Bill To","Sub Total","Ppn","Pph","Grand Total"];
	public $colfields 				= ["id","template_name","po","term","customer","wsubtotal","wppn","wpph","wgrandtotal"];

	/* Form Field Asset */
	public function form_field_asset()
	{
		$field = [];
		$field['txttemplatename'] 	= $this->self_model->return_build_txt('','template_name','template_name');
		...
		$oStatus 					= [['id'=>'1','description' => 'Active'],['id'=>'2','description' => 'Not Active']];
		$oStatus 					= json_decode(json_encode($oStatus), false);
	...
	// Generate invoice template list row
	public function gentinvoiceitemrow()
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1")
		{
			...
			{
				$row = trim($post['count']);
				echo $this->self_model->getNewTInvoiceItemRow($row);
			} else if(isset($post['id'])) {
				$row = 0;
				$id = trim($post['id']);
				$view = (isset($post['view']) && $post['view'] == TRUE)? TRUE:FALSE;
				echo json_encode($this->self_model->getNewTInvoiceItemRow($row,$id,$view));
			}
		}
		else
		{
