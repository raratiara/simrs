...
class Proforma_invoice extends MYP_Controller
{
	/* Costs */
 	const  LABELMODULE				= "finance_proforma_invoice"; // identify menu
 	const  LABELMASTER				= "Proforma Invoice";
 	const  LABELFOLDER				= "finance"; // module folder
 	const  LABELPATH				= "proforma_invoice"; // controller file (lowercase)
	...
	public $tabel_header 			= ["ID","No Proforma Invoice","Tanggal","Term","Due Date","Bill To","Grand Total","Status"];
	
	/* Export */
	public $colnames 				= ["ID","No Proforma Invoice","Tanggal","PO","Term","Due Date","Bill To","Sub Total","Ppn","Pph","Grand Total","Status"];
	public $colfields 				= ["id","pinvoice","dinvoice","po","term","ddue","customer","wsubtotal","wppn","wpph","wgrandtotal","status"];
	...
	public function form_field_asset()
	{
		...
		$field['txtinvoice'] 		= $this->self_model->return_build_txt('Auto','pinvoice','pinvoice','','','readonly');
		$field['txtfaktur'] 		= $this->self_model->return_build_txt('','faktur','faktur');
		$field['worth'] 			= $this->self_model->return_build_txt('0','grandtotal','grandtotal','','text-align: right;','readonly').$this->self_model->return_build_txthidden('0','ppn','ppn').$this->self_model->return_build_txthidden('0','pph','pph').$this->self_model->return_build_txthidden('0','other_tax','other_tax').$this->self_model->return_build_txthidden('0','worth','worth');
	...
	public function getprint($id)
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1" && _USER_ACCESS_LEVEL_DETAIL == "1") {
			...
			// Get item data
			$this->data['rs2'] 	= $this->self_model->getPInvoiceItemRows($pid,TRUE,TRUE);
	...
	public function genpinvoiceitemrow()
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1")
		{
			...
			{
				$row = trim($post['count']);
				echo $this->self_model->getNewPInvoiceItemRow($row);
			} else if(isset($post['id'])) {
				$row = 0;
				$id = trim($post['id']);
				$view = (isset($post['view']) && $post['view'] == TRUE)? TRUE:FALSE;
				echo json_encode($this->self_model->getNewPInvoiceItemRow($row,$id,$view));
			}
		}
		else
		{
