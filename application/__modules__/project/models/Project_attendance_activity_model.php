...
class Project_attendance_activity_model extends MY_Model
{
	/* Module */
 	protected $folder_name			= "project/project_attendance_activity";
	protected $table_name 			= _PREFIX_TABLE."data_project_activity_attendance";
 	protected $table_project 		= _PREFIX_TABLE."data_project";
	protected $table_karyawan 		= _PREFIX_TABLE."data_karyawan"; // Untuk link enginner => belum dipakaim, masih pakai user
	protected $table_kota 			= _PREFIX_TABLE."option_city";
	protected $table_provinsi 		= _PREFIX_TABLE."option_province";
	protected $table_user			= _PREFIX_TABLE."user";
	...
	public function get_list_data()
	{
		$aColumns = [
			...
			'IF(a.approval="1","Approver","Prepared") as approval'
		];

		$sIndexColumn = 'a.'.$this->primary_key;
		$sTable = ' '.$this->table_name.' a 
					LEFT JOIN '.$this->table_project.' b ON a.id_project = b.id
					LEFT JOIN '.$this->table_user.' c ON a.id_engineer = c.user_id
					';
	...
	public function add_data($post) {
		...
		$data = [
			'id_project' 		=> trim($post['id_project']),
			'id_engineer' 		=> $_SESSION["id"],
	...
	public function getRowData($id) { 
		...
		if(!empty($rs->id_engineer)){
			$rd = $this->db->select('name as engineer')->where(["user_id" => $rs->id_engineer])->get($this->table_user)->row();
			$rs = (object) array_merge((array) $rs, (array) $rd);
		} else {
			$rs = (object) array_merge((array) $rs, ['engineer'=>'']);
		}
