...
class Alamat_warehouse_model extends MY_Model
{
	/* Module */
 	protected $folder_name			= "admin/alamat_warehouse";
 	protected $table_province 		= _PREFIX_TABLE."option_province";
	protected $table_city 			= _PREFIX_TABLE."option_city";
 	protected $table_name 			= _PREFIX_TABLE."data_warehouse_address";
	...
	public function get_list_data()
	{
		...
		$sTable = ' '.$this->table_name.' a 
					LEFT JOIN '.$this->table_province.' b ON a.id_province = b.id
					LEFT JOIN '.$this->table_city.' c ON a.id_city = c.id
					';
	...
	public function add_data($post) {
		$id_province = (isset($post['id_province']) && !empty($post['id_province']))? trim($post['id_province']):NULL;
		$id_city = (isset($post['id_city']) && !empty($post['id_city']))? trim($post['id_city']):NULL;
	...
	public function getRowData($id) { 
		...
		if(!empty($rs->id_province)){
			$rd = $this->db->select('description as province')->where([$this->primary_key => $rs->id_province])->get($this->table_province)->row();
			$rs = (object) array_merge((array) $rs, (array) $rd);
		} else {
			$rs = (object) array_merge((array) $rs, ['province'=>'']);
		}
		if(!empty($rs->id_city)){
			$rd = $this->db->select('description as city')->where([$this->primary_key => $rs->id_city])->get($this->table_city)->row();
			$rs = (object) array_merge((array) $rs, (array) $rd);
		} else {
			$rs = (object) array_merge((array) $rs, ['city'=>'']);
		}
	...
	public function eksport_data()
	{
		$sql = "SELECT
					...
					IF(a.active='1','Yes','No') as is_active
			FROM
	   	 		".$this->table_name." a LEFT JOIN ".$this->table_province." b ON a.id_province = b.id 
	   		ORDER BY
