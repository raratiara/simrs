...
class Proforma_invoice_model extends MY_Model
{
	/* Module */
 	protected $folder_name			= "finance/proforma_invoice";
 	protected $table_name 			= _PREFIX_TABLE."data_proforma_invoice";
	...
	protected $qrcode_assets 		= './assets/images/qrcode/pinvoice/';
	...
	public function add_data($post) {
		...
		$nextnum 	= 'P'.$branch.$lettercode.$yearcode.str_pad($runningnumber, 5, "0", STR_PAD_LEFT);
		// EOF auto numbering 
		// BOF qrcode 
		$qrdata = $nextnum;
	...
	public function getNewPInvoiceItemRow($row,$id=0,$view=FALSE)
	{ 
		if($id > 0){
			$data = $this->getPInvoiceItemRows($id,$view);
		} else {
			$data = '';
			$no = $row+1;
	...
	public function getPInvoiceItemRows($id,$view,$print=FALSE){
		$dt = '';
