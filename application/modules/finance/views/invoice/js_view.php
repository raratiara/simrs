...
var vppn = 0; // for ppn tarif ratio holder
var vdpp = 0; // for ppn tarif ratio holder
        ...
        success: function(data)
        {
			if(data != false){
				if(save_method == 'update'){
					$('.tinvoice').hide();
					$('.proinvoice').hide();
					$.ajax({type: 'post',url: module_path+'/getppntarrif',data: { udate:data.dinvoice },success: function (response) {
							vppn = response;
							$('[name="coretax"]').val(vppn);
							$('[name="id"]').val(data.id);
							...
							$.ajax({type: 'post',url: module_path+'/geninvoiceitemrow',data: { id:data.id },success: function (response) {
				...
				if(save_method == 'detail'){
					...
					$('span.invoice').html(data.invoice);
					...
					$.ajax({type: 'post',url: module_path+'/geninvoiceitemrow',data: { id:data.id,view:true },success: function (response) {
							var obj = JSON.parse(response);
...
$( "#btnAddData" ).on('click', function(){
	...
	$.ajax({type: 'post',url: module_path+'/getppntarrif',data: { udate:tdate },success: function (response) {
			vppn = response;
			$('.proinvoice').show();
			$('.tinvoice').show();
...
$('#frmInputData').on('change', 'select#id_tinvoice', function(event){
	...
	$.ajax({type: 'post',url: module_path+'/gettinvoiceinfo',data: { sel:id },success: function (response) {
			...
			$.ajax({type: 'post',url: module_path+'/gentinvoiceitemrow',data: { id:obj.id },success: function (response) {
...
/* Performa Invoice load */
$('#frmInputData').on('change', 'select#id_proinvoice', function(event){
	var id= $(this).val();
	$.ajax({type: 'post',url: module_path+'/getproinvoiceinfo',data: { sel:id },success: function (response) {
			var obj = JSON.parse(response);
			$('[name="dpp"]').val(obj.dpp);
			$('[name="ppn"]').val(obj.ppn);
			$('[name="pph"]').val(obj.pph);
			...
			$('[name="notes"]').val(obj.notes);
			$('select#id_status').val('1').trigger('change.select2');
			$('[name="faktur"]').val(obj.faktur);
			$.ajax({type: 'post',url: module_path+'/genpinvoiceitemrow',data: { id:obj.id },success: function (response) {
					var objc = JSON.parse(response);
					$(locate+' tbody').html(objc[0]);
					$(locate+' input.qty').number(true,0,',','.');
					$(locate+' input.price').number(true,0,',','.');
...
$('#frmInputData').on('change', 'select#id_po', function(event){
	...
	$.ajax({type: 'post',url: module_path+'/getpoinfo',data: { sel:id },success: function (response) {
			...
			$('[name="description"]').val(obj.description);
			$('[name="notes"]').val(obj.notes);
			$.ajax({type: 'post',url: module_path+'/genpoitemrow',data: { id:obj.id },success: function (response) {
					var objc = JSON.parse(response);
					$(locate+' tbody').html(objc[0]);
					$(locate+' input.qty').number(true,0,',','.');
					$(locate+' input.price').number(true,0,',','.');
...
// BOF operation
$('#frmInputData').on('change', 'input#date_invoice', function(event){
	var udate= $(this).val();
	$.ajax({type: 'post',url: module_path+'/getppntarrif',data: { udate:udate },success: function (response) {
			vppn = response;
			$(opsForm+' [name="coretax"]').val(vppn);
			updatetotal();
		}
	});
});
...
$("#addrow").on("click", function () {
	expire();
	var newRow = $("<tr>");
	$.ajax({type: 'post',url: module_path+'/geninvoiceitemrow',data: { count:wcount },success: function (response) {
