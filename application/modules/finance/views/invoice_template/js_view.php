...
var modloc = '/finance/invoice_template/';
...
jQuery(function($) {
	...
	.DataTable( {
		...
		"aoColumnDefs": [
		  ...
		  { "sClass": "dt-body-right", "aTargets": [ 6 ] }
		],
		"aaSorting": [
		  	[2,'asc'] 
		],
			...
			if(data != false){
				if(save_method == 'update'){
					...
					$.ajax({type: 'post',url: module_path+'/getppntarrif',data: { udate:tdate },success: function (response) {
							...
							$.ajax({type: 'post',url: modloc+'gentinvoiceitemrow',data: { id:data.id },success: function (response) {
				...
				if(save_method == 'detail'){
					$('[name="id"]').val(data.id);
					$('span.template_name').html(data.template_name);
					...
					$.ajax({type: 'post',url: modloc+'gentinvoiceitemrow',data: { id:data.id,view:true },success: function (response) {
							var obj = JSON.parse(response);
...
$("#addrow").on("click", function () {
	expire();
	var newRow = $("<tr>");
	$.ajax({type: 'post',url: modloc+'gentinvoiceitemrow',data: { count:wcount },success: function (response) {
