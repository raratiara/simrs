...
{
	/* Module */
 	protected $folder_name			= "master/uom";
 	protected $table_name 			= _PREFIX_TABLE."option_uom";
 	protected $table_group 			= _PREFIX_TABLE."option_uom_group";
	...
	public function get_list_data()
	{
		...
		$sTable = ' '.$this->table_name.' a
					LEFT JOIN '.$this->table_group.' b ON b.id=a.idx_group
				';
	...
	public function add_data($post) {
		...
		$idx_group = (isset($post['idx_group']) && !empty($post['idx_group']))? trim($post['idx_group']):NULL;
	...
	public function getRowData($id) { 
		...
		if(!empty($rs->idx_group)){
			$rd = $this->db->select('description as group')->where([$this->primary_key => $rs->idx_group])->get($this->table_group)->row();
			$rs = (object) array_merge((array) $rs, (array) $rd);
		} else {
			$rs = (object) array_merge((array) $rs, ['group'=>'']);
		}
	...
	public function import_data($list_data)
	{
		...
		foreach ($list_data as $k => $v) {
			...
			$idx_group = !empty($v["D"])? trim($v["D"]):NULL;
			$idx_seq = !empty($v["E"])? trim($v["E"]):'0';
			$active = !empty($v["F"])? trim($v["F"]):'0';
			$data = [
				'code' 				=> $v["B"],
				'description' 		=> $v["C"],
				'idx_seq' 			=> $idx_seq,
	...
	public function eksport_data()
	{
		$sql = "SELECT
			...
			FROM
	   	 		".$this->table_name." a
			LEFT JOIN 
				".$this->table_group." b ON b.id=a.idx_group	
	   		ORDER BY
