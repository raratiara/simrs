...
class Sadmin_tools_access_menu extends CI_Controller
{
	/* Module */
 	private $folder_name			= "general_system/sadmin_tools_access_menu";
 	private $module_name			= "general_system_sadmin_tools_access_menu";
 	private $table_name 			= "bright_user";
 	private $primary_key 			= "user_id";
 	private $model_name				= "sadmin_tools_access_menu_model";
	/* Navigation */
 	private $parent_menu			= "general_system";
 	private $subparent_menu			= "general_system_sadmin";
 	private $sub_menu 				= "general_system_sadmin_tools_access_menu";
	/* Label */
   	...
   	public function __construct()
	{
		...
		# akses level
		$akses = $this->sadmin_tools_access_menu_model->user_akses($this->module_name);
	...
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1") {
			$this->sadmin_tools_access_menu_model->get_list_data();
		} else {
			$this->load->view('errors/html/error_hacks_401');
		}
	}
	...
	public function delete($id)
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1" && _USER_ACCESS_LEVEL_DELETE == "1") {
			$rs = $this->sadmin_tools_access_menu_model->delete($id);
	...
	public function add()
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1" && _USER_ACCESS_LEVEL_ADD == "1") {
			...
			{
				$rs = $this->sadmin_tools_access_menu_model->add_data($post);
				...
				$this->data['seluser'] 		= $this->sadmin_tools_access_menu_model->return_build_select2me($oUserAccount,'','','','user_id','user_id','','','user_id','name','-','username');
	...
	public function edit($id)
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1" && _USER_ACCESS_LEVEL_UPDATE == "1") {
			if ($post = $this->input->post())
			{
				$rs = $this->sadmin_tools_access_menu_model->edit_data($post,$id);
				...
				$rs 						= $this->sadmin_tools_access_menu_model->getRowData($id);
				...
				$oUserAccount 				= $this->db->query($sQuery)->result();
				$this->data['seluser'] 		= $this->sadmin_tools_access_menu_model->return_build_select2me($oUserAccount,'',$rs->user_id,'','user_id','user_id','','','user_id','name','-','username','disabled');
				$rs 						= $this->sadmin_tools_access_menu_model->getRowDataAccess($id);
				if (count($rs) > 0) {
					FOREACH ($rs AS $r) {
	...
	public function import_action()
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1" && _USER_ACCESS_LEVEL_IMPORT == "1") {
			...
			if (! $this->upload->do_upload("userfile")) {
				...
				# upload to database
				$rs = $this->sadmin_tools_access_menu_model->import_data($tmp_data);
			}
	...
	public function eksport_action()
	{
		if(_USER_ACCESS_LEVEL_VIEW == "1" && _USER_ACCESS_LEVEL_EKSPORT == "1") {
			...
			$data 	= $this->sadmin_tools_access_menu_model->eksport_data();
			...
			$this->sadmin_tools_access_menu_model->export_to_excel($colnames,$colfields, $data, $header ,"rpt_sadmin_tools_access_menu");
		} else {
			$this->load->view('errors/html/error_hacks_401');
		}
	}
