        ...
        success: function(data)
        {
			if(data != false){
				if(save_method == 'update'){
					...
					$('[name="date_spk"]').val(data.dspk);
					$('[name="date_due"]').val(data.ddue);
					$('[name="id_customer"]').val(data.id_customer).trigger('change.select2');
				...
				if(save_method == 'detail'){
					...
					$('span.dspk').html(data.dspk);
					$('span.ddue').html(data.ddue);
					$('span.customer').html(data.customer);
...
$('form#frmInputData').on('keyup', '[name="worth"]', function(event){
	$(this).number(true,0,',','.');
});
