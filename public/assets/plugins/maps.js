jQuery(document).ready(function($) {
    ...
    function restartInterval() {
        ...
        if (viewingAllRoutes) {
            intervalID = setInterval(getAllRoutesForMap, 3 * 1000); // one minute 
        } else {
            intervalID = setInterval(getRouteForMap, 3 * 1000);          
        }          
        // alert(intervalID);
    }
    ...
    function deleteRoute() {
        if (hasMap()) {
            ...
            if (answer){
				url: baseurl +'tracker/deleteroute/' + $('#routeSelect').val(); 
